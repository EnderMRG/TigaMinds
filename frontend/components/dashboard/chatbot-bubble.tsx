  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-20 right-4 sm:bottom-6 sm:right-6 z-40 p-3.5 sm:p-4 rounded-full bg-primary text-primary-foreground shadow-xl hover:shadow-2xl transition-all duration-300 hover:scale-105 active:scale-95"
      ...
      {isOpen && (
        <Card className="fixed inset-x-3 bottom-20 sm:inset-x-auto sm:bottom-24 sm:right-6 z-40 w-auto sm:w-[400px] shadow-2xl rounded-2xl overflow-hidden flex flex-col max-h-[calc(100vh-140px)] sm:max-h-[600px] h-[75vh] bg-background gap-0 py-0 border border-border animate-in slide-in-from-bottom-5 fade-in zoom-in-95 duration-200">
          <div className="bg-primary text-primary-foreground p-3.5 sm:p-4 flex justify-between items-center shrink-0 border-b border-border">
            <div>
            ...
            <button onClick={() => setIsOpen(false)} className="text-primary-foreground/80 hover:text-primary-foreground hover:bg-primary-foreground/10 p-1.5 rounded-full transition-colors" aria-label="Close assistant">
              <X className="h-5 w-5" />
            </button>
          </div>
          <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-3.5 bg-background scrollbar-thin">
            {messages.map((message) => (
              <div key={message.id} className={`flex ${message.type === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[88%] px-3.5 py-2.5 sm:px-4 sm:py-3 rounded-2xl shadow-sm ${message.type === 'user' ? 'bg-primary text-primary-foreground rounded-br-none' : 'bg-muted text-foreground rounded-bl-none border border-border'}`}>
            ...
            {isLoading && (
              <div className="flex justify-start">
                <div className="bg-muted border border-border px-3.5 py-2.5 rounded-2xl rounded-bl-none flex items-center gap-2 text-foreground shadow-sm">
          ...
          <div className="bg-muted/30 border-t border-border p-3 sm:p-4 shrink-0">
            <div className="flex overflow-x-auto sm:flex-wrap gap-1.5 sm:gap-2 justify-start sm:justify-center mb-3 pb-1 scrollbar-none">
              {quickActions.map(action => (
                <button 
                  key={action.id}
                  onClick={() => handleQuickAction(action.id)}
                  className="bg-background border border-border hover:border-primary/50 hover:bg-muted text-[11px] sm:text-xs px-2.5 py-1.5 rounded-full shadow-sm transition-all duration-200 flex items-center gap-1.5 flex-shrink-0"
                >
                  <action.icon className="h-3 w-3 sm:h-3.5 sm:w-3.5 text-primary" />
            ...
            <div className="flex gap-2">
              <input type="text" value={inputValue} onChange={(e) => setInputValue(e.target.value)}
                ...
                className="flex-1 px-3.5 py-2 bg-background border border-border text-foreground rounded-full focus:outline-none focus:ring-2 focus:ring-primary/50 text-xs sm:text-sm shadow-inner" />
              <Button onClick={handleSendMessage} disabled={!inputValue.trim() || isLoading} className="rounded-full h-9 w-9 sm:h-10 sm:w-10 p-0 shadow-md shrink-0">
